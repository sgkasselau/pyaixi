...
# -*- coding: utf-8 -*-
"""
Defines a class for Monte Carlo-AIXI agents, using a pluggable model instance to make predictions.
"""
...
class MC_AIXI_Undo:
    """ A class to save details from a MC-AIXI agent to restore state later.
    """
    ...
    def __init__(self, agent):
        ...
        self.mean_reward                 = agent.mean_reward
        self.reward_m2                   = agent.reward_m2
        self.total_reward                = agent.total_reward
    # end def
# end class


class MC_AIXI_Agent(agent.Agent):
    """ This class represents a Monte Carlo-AIXI agent.

        It includes much of the high-level logic for choosing suitable actions.
        In particular, the agent maintains an internal model of the environment in `model`.
    ...
    def __init__(self, environment = None, model = None, options = {}):
        """ Construct a Monte Carlo-AIXI learning agent from the given configuration values, the environment,
            and a model.

             - `environment` is an instance of the pyaixi.Environment class that the agent with interact with.
             - `model` is an instance of the pyaixi.Model class used to make predictions
        ...
        # Store the given model, if it's valid.
        assert model is not None, "The model given to MC_AIXI_Agent is invalid."
        self.model = model
    ...
    def generate_action(self):
        """ Returns an action generated according to the agent's history
            statistics by doing rejection sampling from the model.
        ...
        # Sample from the model, getting a list of symbols corresponding to an action.
        # NOTE: works only for a binary symbol alphabet.
        action_list = self.model.generate_random_symbols(self.environment.action_bits())
    ...
    def generate_percept(self):
        """ Returns a percept (an observation, reward pair) distributed according to the agent's history
            statistics by sampling from the model.

            (Called `genPercept` in the C++ version.)
        """

        # Sample from the model.
        percept_symbols = self.model.generate_random_symbols(self.environment.percept_bits())

        # Decode and return the percept symbols into the desired observation and reward.
        return self.decode_percept(percept_symbols)
    # end def

    def generate_percept_and_update(self):
        """ Returns a percept (an observation, reward pair) distributed according to the agent's history
            statistics, after updating the model with it.

            (Called `genPerceptAndUpdate` in the C++ version.)
        """

        # Sample from the model.
        percept_symbols = self.model.generate_random_symbols_and_update(self.environment.percept_bits())
    ...
    def get_predicted_action_probability(self, action):
        ...
        # Get the probability of seeing this action using the model to make a prediction.
        return self.model.predict(action_symbols)
    # end def
    ...
    def model_revert(self, undo_instance):
        ...
        while self.history_size() > undo_instance.history_size:
            ...
            if self.last_update == percept_update:
                # Undo a percept.
                self.model.revert(self.environment.percept_bits())
                self.last_update = action_update
            else:
                # Undo an action.
                self.model.revert_history(self.environment.action_bits())
                self.last_update = percept_update
            # end if
        # end while
        assert self.history_size() == undo_instance.history_size, \
               "Model revert couldn't revert the right number of symbols. (%d != %d)" % \
               (self.history_size(), undo_instance.history_size)

        assert self.last_update == undo_instance.last_update, \
               "Model revert put back model in the wrong update state '%s' != '%s'." % \
               (self.last_update, undo_instance.last_update)

        # Revert agent parameters.
        self.age                         = undo_instance.age
        self.last_update                 = undo_instance.last_update
        self.mean_reward                 = undo_instance.mean_reward
        self.reward_m2                   = undo_instance.reward_m2
    ...
    def model_update_action(self, action):
        ...
        # Let the parent class perform certain model updates for this action first.
        agent.Agent.model_update_action(self, action)

        # Get the symbols that represent this action.
        action_symbols = self.encode_action(action)

        # Update the model.
        self.model.update_history(action_symbols);
    # end def
    ...
    def model_update_percept(self, observation, reward):
        ...
        # Let the parent class perform certain model updates for this percept first.
        agent.Agent.model_update_percept(self, observation, reward)
        ...
        if ((self.learning_period > 0) and (self.age > self.learning_period)):
            ...
            self.model.update_history(percept_symbols)
        else:
            # Yes. Update and learn.
            self.model.update(percept_symbols)
        # end if
    # end def
    ...
    def percept_probability(self, observation, reward):
        ...
        # Get the probability of seeing this percept using the model to make a prediction.
        return self.model.predict(percept_symbols)
    # end def
    ...
    def reset(self):
        """ Resets the agent and clears the model.
        """
    ...
    def search(self):
        ...
        undo_instance = MC_AIXI_Undo(self)
        ...
        # Find the best action from those evaluated by the search tree.
        for action in search_tree.children:
